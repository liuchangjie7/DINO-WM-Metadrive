...
class PlanWorkspace:
    def __init__(
        ...
        self.data_preprocessor = Preprocessor( # 数据归一化处理
    ...
    def prepare_targets(self): # 准备规划目标
        states = []
        actions = []
        observations = []
...
def planning_main(cfg_dict):
    ...
    ckpt_base_path = cfg_dict["ckpt_base_path"] # 模型权重存放根目录
    model_path = f"{ckpt_base_path}/outputs/{cfg_dict['model_name']}/" # 模型权重存放目录
    with open(os.path.join(model_path, "hydra.yaml"), "r") as f: # 读取模型训练配置文件
        model_cfg = OmegaConf.load(f)

    seed(cfg_dict["seed"])
    _, dset = hydra.utils.call( # 加载数据集
    ...
    num_action_repeat = model_cfg.num_action_repeat # 动作重复次数
    model_ckpt = (
        Path(model_path) / "checkpoints" / f"model_{cfg_dict['model_epoch']}.pth"
    ) # 模型权重路径
    model = load_model(model_ckpt, model_cfg, num_action_repeat, device=device) # 加载模型
    ...
    logs = plan_workspace.perform_planning() # 执行规划
    return logs

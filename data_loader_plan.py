...
class MetaDriveDataset(Dataset):
    def __init__(self, files, num_hist, num_pred, frameskip=1):
        ...
        # 【关键修复】计算统计量并定义 transform
        self.compute_stats()

    def compute_stats(self):
        # 1. 定义 transform 为“什么都不做”，防止 preprocessor 报错
        # 因为我们在 __getitem__ 和 env wrapper 里已经手动做了 /255.0
        self.transform = lambda x: x

        # 2. 计算均值方差 (Planner 需要这些来反归一化动作)
        sample_files = self.files[:100]
        all_acts = []
        all_states = []

        # print("Computing dataset statistics (sampling)...")
    ...
    def __getitem__(self, idx):
        ...
        # 返回 4 个值以防万一 (obs, act, state, info)
...
class MetaDriveTrajectoryDataset(Dataset):
    ...
    def compute_stats(self):
        self.transform = lambda x: x  # 防止报错

        # 简单处理，直接设为默认值或者重新采样
        # 这里的统计量主要给 Planner 用，通常用训练集的就够了，
        # 但为了防止 AttributeError，这里给个默认值或简单算一下
        self.action_mean = torch.zeros(self.action_dim)
        self.action_std = torch.ones(self.action_dim)
        self.state_mean = torch.zeros(self.proprio_dim)
        self.state_std = torch.ones(self.proprio_dim)
